...
const TIMELINE = [
  ...
  { frame: 120, type: "sound-line" as const, text: '🔊 "Captain on the bridge."', sound: "wc2ships-human1.wav", label: "— session started" },
  ...
  { frame: 240, type: "sound-line" as const, text: '🔊 "Under way."', sound: "wc2ships-acknowledge3.wav", label: "— reading files" },
  { frame: 290, type: "line" as const, text: "  [you switch to Slack]", style: "dim" as const },
  { frame: 330, type: "sound-line" as const, text: '🔊 "Skipper?"', sound: "wc2ships-human3.wav", label: "— permission needed" },
  ...
  { frame: 470, type: "sound-line" as const, text: '🔊 "Aye aye, sir."', sound: "wc2ships-acknowledge1.wav", label: "— analyzing code" },
  { frame: 530, type: "sound-line" as const, text: '🔊 "Set sail?"', sound: "wc2ships-human4.wav", label: "— task complete" },
  ...
  { frame: 680, type: "sound-line" as const, text: '🔊 "Stop rocking the boat!"', sound: "wc2ships-annoyed1.wav", label: "— error" },
  { frame: 740, type: "outro" as const },
];

// WC2 nautical palette
const BG = "#1a1b26";
const BAR_BG = "#0c0d14";
const GREEN = "#4ade80";
const WC3_GOLD = "#ffab01";
const WC3_GOLD_DIM = "rgba(255, 171, 1, 0.3)";
const OCEAN_TEAL = "#0891b2";
const DIM = "#505a79";
const BRIGHT = "#e0e8ff";
...
const TerminalChrome: React.FC<{ children: React.ReactNode; tabTitle: string }> = ({ children, tabTitle }) => (
  <div style={{ width: 940, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(8,145,178,0.15)", boxShadow: "0 0 60px rgba(0,0,0,0.6), 0 0 4px rgba(8,145,178,0.1)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", backgroundColor: BAR_BG, borderBottom: "1px solid rgba(8,145,178,0.1)" }}>
...
const TitleCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center", opacity: exitOp }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${OCEAN_TEAL}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${OCEAN_TEAL}, transparent)` }} />
      <div style={{ position: "absolute", top: "50%", left: "50%", width: 600, height: 600, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(8,145,178,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", opacity: titleSpring, transform: `translateY(${interpolate(titleSpring, [0, 1], [20, 0])}px)` }}>
        <div style={{ fontFamily: "monospace", fontSize: 20, color: OCEAN_TEAL, letterSpacing: 6, textTransform: "uppercase", marginBottom: 16, opacity: subSpring }}>sound pack</div>
        <div style={{ fontFamily: "Georgia, 'Palatino Linotype', serif", fontSize: 72, fontWeight: 700, color: "#fff", textShadow: "3px 3px 0 rgba(0,0,0,0.8)", textAlign: "center", lineHeight: 1.2 }}>WC2 Human Ships</div>
...
const OutroCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${OCEAN_TEAL}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${OCEAN_TEAL}, transparent)` }} />
      <div style={{ position: "absolute", top: "40%", left: "50%", width: 500, height: 500, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(8,145,178,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
...
export const Wc2HumanShipsPreview: React.FC = () => {
  const frame = useCurrentFrame();
    ...
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f" }}>
                ...
                return (
                  <TermLine key={i} appearFrame={event.frame}>
                    {event.style === "cmd" ? (
                      ...
                      <span style={{ color: OCEAN_TEAL }}>{event.text}</span>
                    ) : event.style === "cursor" ? (
      ...
      {TIMELINE.filter((e) => e.type === "sound-line" && e.sound).map((event, i) => {
        const durations: Record<string, number> = {
          "wc2ships-human1.wav": 69,
          "wc2ships-acknowledge3.wav": 58,
          "wc2ships-human3.wav": 47,
          "wc2ships-acknowledge1.wav": 60,
          "wc2ships-human4.wav": 56,
          "wc2ships-annoyed1.wav": 87,
        };
