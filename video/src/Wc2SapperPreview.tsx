...
const TIMELINE = [
  ...
  { frame: 120, type: "sound-line" as const, text: '🔊 "We\'re ready."', sound: "wc2sapper-ready.wav", label: "— session started" },
  ...
  { frame: 240, type: "sound-line" as const, text: '🔊 "Yes, boss."', sound: "wc2sapper-yes_boss.wav", label: "— reading files" },
  { frame: 290, type: "line" as const, text: "  [you switch to Slack]", style: "dim" as const },
  { frame: 330, type: "sound-line" as const, text: '🔊 "Hello?"', sound: "wc2sapper-hello_question.wav", label: "— permission needed" },
  ...
  { frame: 470, type: "sound-line" as const, text: '🔊 "We\'ve got explosives!"', sound: "wc2sapper-weve_got_explosives.wav", label: "— analyzing code" },
  { frame: 530, type: "sound-line" as const, text: '🔊 "Oooh it\'s beautiful!"', sound: "wc2sapper-its_beautiful.wav", label: "— task complete" },
  ...
  { frame: 680, type: "sound-line" as const, text: '🔊 "KABOOOOM!"', sound: "wc2sapper-kaboom.wav", label: "— error" },
  { frame: 740, type: "outro" as const },
];

// Goblin green palette
const BG = "#1a1b26";
const BAR_BG = "#0c0d14";
const GREEN = "#4ade80";
const WC3_GOLD = "#ffab01";
const WC3_GOLD_DIM = "rgba(255, 171, 1, 0.3)";
const GOBLIN_GREEN = "#15803d";
const DIM = "#505a79";
const BRIGHT = "#e0e8ff";
...
const TerminalChrome: React.FC<{ children: React.ReactNode; tabTitle: string }> = ({ children, tabTitle }) => (
  <div style={{ width: 940, borderRadius: 12, overflow: "hidden", border: "1px solid rgba(21,128,61,0.15)", boxShadow: "0 0 60px rgba(0,0,0,0.6), 0 0 4px rgba(21,128,61,0.1)" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", backgroundColor: BAR_BG, borderBottom: "1px solid rgba(21,128,61,0.1)" }}>
...
const TitleCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center", opacity: exitOp }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${GOBLIN_GREEN}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${GOBLIN_GREEN}, transparent)` }} />
      <div style={{ position: "absolute", top: "50%", left: "50%", width: 600, height: 600, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(21,128,61,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", opacity: titleSpring, transform: `translateY(${interpolate(titleSpring, [0, 1], [20, 0])}px)` }}>
        <div style={{ fontFamily: "monospace", fontSize: 20, color: GOBLIN_GREEN, letterSpacing: 6, textTransform: "uppercase", marginBottom: 16, opacity: subSpring }}>sound pack</div>
        <div style={{ fontFamily: "Georgia, 'Palatino Linotype', serif", fontSize: 72, fontWeight: 700, color: "#fff", textShadow: "3px 3px 0 rgba(0,0,0,0.8)", textAlign: "center", lineHeight: 1.2 }}>WC2 Goblin Sappers</div>
        <div style={{ fontFamily: "Georgia, serif", fontSize: 36, color: "rgba(255,255,255,0.5)", marginTop: 12, opacity: subSpring }}>Warcraft II: Tides of Darkness</div>
      </div>

      <div style={{ position: "absolute", bottom: 40, left: 40, display: "flex", alignItems: "center", gap: 8, opacity: subSpring }}>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 700, color: "rgba(255,255,255,0.5)" }}>X</span>
...
const OutroCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f", justifyContent: "center", alignItems: "center" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${GOBLIN_GREEN}, transparent)` }} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, transparent, ${GOBLIN_GREEN}, transparent)` }} />
      <div style={{ position: "absolute", top: "40%", left: "50%", width: 500, height: 500, transform: "translate(-50%, -50%)", borderRadius: "50%", background: "radial-gradient(circle, rgba(21,128,61,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
...
export const Wc2SapperPreview: React.FC = () => {
  const frame = useCurrentFrame();
    ...
    <AbsoluteFill style={{ backgroundColor: "#0a0a0f" }}>
      ...
      {termEnter && (
        <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", opacity: termOp }}>
                ...
                return (
                  <TermLine key={i} appearFrame={event.frame}>
                    {event.style === "cmd" ? (
                      ...
                      <span style={{ color: GOBLIN_GREEN }}>{event.text}</span>
                    ) : event.style === "cursor" ? (
          ...
          <div style={{ position: "absolute", bottom: 30, left: 40, display: "flex", alignItems: "center", gap: 10, opacity: 0.6 }}>
            ...
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 15, fontWeight: 700, color: "rgba(255,255,255,0.5)" }}>X</span>
      ...
      {TIMELINE.filter((e) => e.type === "sound-line" && e.sound).map((event, i) => {
        const durations: Record<string, number> = {
          "wc2sapper-ready.wav": 55,
          "wc2sapper-yes_boss.wav": 58,
          "wc2sapper-hello_question.wav": 51,
          "wc2sapper-weve_got_explosives.wav": 76,
          "wc2sapper-its_beautiful.wav": 82,
          "wc2sapper-kaboom.wav": 64,
        };
